...
def train(model, data_loader, optimizer, epoch, device, config):
    ...
    metric_logger.add_meter('loss_ra', utils.SmoothedValue(window_size=1, fmt='{value:.4f}'))
    metric_logger.add_meter('loss_lm', utils.SmoothedValue(window_size=1, fmt='{value:.4f}'))
    metric_logger.add_meter('loss_sa', utils.SmoothedValue(window_size=1, fmt='{value:.4f}'))
    header = 'Train Epoch: [{}]'.format(epoch)
    print_freq = 50

    for i, (image1, image2, caption1, caption2, person, replace) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        loss_ita, loss_itm, loss_ra, loss_lm, loss_sa = model(image1, image2, caption1, caption2, alpha=alpha, idx=person, replace=replace)
        loss = loss_ita + loss_itm + loss_lm
        for j, los in enumerate((loss_ra, loss_sa)):
        ...
        metric_logger.update(loss_ra=loss_ra.item())
        metric_logger.update(loss_lm=loss_lm.item())
        metric_logger.update(loss_sa=loss_sa.item())
...
def main(args, config):
    ...
    train_dataset, val_dataset, test_dataset = create_dataset('ps_pretrain', config)
