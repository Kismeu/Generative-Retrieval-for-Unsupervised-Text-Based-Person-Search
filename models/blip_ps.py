...
class BLIP_PS(nn.Module):
    def __init__(self,
                 med_config='configs/med_config.json', # 混合编码器-解码器模型的配置文件路径
                 image_size=384,  # 输入图像的尺寸
                 vit='base',
                 vit_grad_ckpt=False,
                 vit_ckpt_layer=0,
                 embed_dim=256,   # 嵌入空间的维度
                 queue_size=57600,  # 用于对比学习的队列大小
                 momentum=0.995,  # 动量编码器的动量参数
                 max_length=50,   # tokenizer的最大长度
                 ):
        """
        Args:
    ...
    def forward(self, image1, image2, caption1, prob, alpha, idx, confident=True, probability_matrix=None, vocab_size=None):
        """
        Args:
            prob (Tensor):
        ...
        if confident:
            ...
            loss_i2t = -torch.sum(F.log_softmax(sim_i2t, dim=1) * sim_i2t_targets * prob_all, dim=1).mean()
            loss_t2i = (-torch.sum(F.log_softmax(sim_t2i, dim=1) * sim_t2i_targets, dim=1) * prob).mean()
        ...
        prob_all = torch.cat([prob, prob, prob_neg], )
        ...
        if confident:
            ...
            loss_itm = (F.cross_entropy(vl_output, itm_labels, reduction='none') * prob_all).mean()
